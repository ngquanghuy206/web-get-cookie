...
╚══════╝░╚════╝░░╚════╝░░╚════╝░""".format(phone))
	...
	def sendCardBank(self, message_object, banknum, nameAccBank,bank, thread_id, thread_type):
		"""Gửi một card bank đến người dùng hoặc nhóm.
		
		Args:
			message_object (Message): Message object gốc
			banknum (str): Nội dung todo
			assignees (list): Danh sách ID người nhận
			thread_id (str): ID của thread 
			thread_type (ThreadType): Loại thread (USER/GROUP)
			due_date (int): Thời hạn (-1 nếu không có)
		...
		if not banknum:
			raise ZaloAPIException("Missing todo content")
		if not nameAccBank:
			raise ZaloAPIException("Missing Bank Author Name")
		if not bank:
			raise ZaloAPIException("Missing Bank")
		bank = bank.upper()
		bin_code = None
		for banks in banks_bin:
			if bank in banks:
				bin_code = banks[bank]
				break
		if not bin_code:
			raise ZaloAPIException("Unknown Bank")
		params = {
			"zpw_ver": 647,
			"zpw_type": 30
		}
		...
		is_group = thread_type == ThreadType.GROUP
		t_type = "1" if thread_type == ThreadType.GROUP else "0"
		payload = {
			"params": {
				"binBank": bin_code,
				"numAccBank": banknum,
	...
	def createReminder(self, title, description, time_ms, groupId, members=None):
		"""Tạo reminder (nhắc nhở) trong nhóm.
		
		Args:
			title (str): Tiêu đề của reminder
			description (str): Mô tả của reminder
			time_ms (int): Thời gian nhắc nhở (milliseconds)
			groupId (int | str): ID của nhóm
			members (list): Danh sách member ID được nhắc (mặc định: [])
			
		Returns:
			dict: Thông tin reminder vừa tạo
		...
		payload["params"] = self._encode(payload["params"])
		response = self._post("https://groupboard-wpa.chat.zalo.me/api/board/v1/group/board/create", params=params, data=payload)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
		...
		error_code = data.get("error_code")
		error_message = data.get("error_message") or data.get("data")
		raise ZaloAPIException(f"Error #{error_code} when creating reminder: {error_message}")

	def sendLocation(self, latitude, longitude, thread_id, thread_type, title=None, address=None, ttl=0):
		"""Gửi vị trí (location) đến người dùng hoặc nhóm.
		
		Args:
			latitude (float): Vĩ độ
			longitude (float): Kinh độ
			thread_id (int | str): User/Group ID để gửi đến
			thread_type (ThreadType): ThreadType.USER hoặc ThreadType.GROUP
			title (str): Tên địa điểm (mặc định: "latitude, longitude")
			address (str): Địa chỉ hiển thị (mặc định: "Lat: ..., Lng: ...")
			ttl (int): Time to live (mặc định: 0)
			
		Returns:
			object: `User` nếu gửi đến user, `Group` nếu gửi đến nhóm
			
		Raises:
			ZaloUserError: Nếu thread_type không hợp lệ
			ZaloAPIException: Nếu request thất bại
		"""
		params = {
			"zpw_ver": 645,
		...
		location_title = title or f"{latitude}, {longitude}"
		location_desc = address or f"Lat: {latitude}, Lng: {longitude}"
